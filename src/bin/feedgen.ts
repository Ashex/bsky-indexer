...
const main = async (): Promise<void> => {
	...
	const dbOptions = {
		...
		poolSize: env.poolSize ?? 50, // Lower pool size for feedgen-only indexer
	...
	const sub = new FirehoseSubscription({
		...
		redisSeqKey: "bsky_feedgen_indexer:seq", // Separate cursor for feedgen indexer
		...
		feedgenAutoDiscovery: env.feedgenAutoDiscovery === "true" || env.feedgenAutoDiscovery === "1",
		onError: (err) => console.error(...(err.cause ? [err.message, err.cause] : [err])),
	}, new URL("./feedgen-worker.ts", import.meta.url));
...
const getEnv = () => ({
	...
	maxTimestampDeltaMs: process.env.SUB_MAX_TIMESTAMP_DELTA_MS
		...
		: undefined,
	statsFrequencyMs: process.env.SUB_STATS_FREQUENCY_MS
		? parseInt(process.env.SUB_STATS_FREQUENCY_MS)
		: undefined,
	feedgenAutoDiscovery: process.env.FEEDGEN_AUTO_DISCOVERY || undefined,
});
